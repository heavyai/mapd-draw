...
import { buildXformMatrix } from "../shapes/transform2d"
...
function drawOBBoxModifierRect(
  ctx,
  pt,
  objToScreenMat,
  modifierSize,
  ...
  ctx.translate(-pt[0], -pt[1])
  ctx.rect(
    pt[0] - modifierHalfSize,
    pt[1] - modifierHalfSize,
    modifierSize,
    modifierSize
  )
...
function isPointInOBBoxModifierRect(
  screenPt,
  modifierPt,
  objToScreenMat,
  modifierMat,
  ...
  if (Point2d.distance(modifierPt, screenPt) <= padBoxRadius) {
    ...
    modifierMat[4] =
      modifierMat[0] * modifierPt[0] + modifierMat[2] * modifierPt[1]
    modifierMat[5] =
      modifierMat[1] * modifierPt[0] + modifierMat[3] * modifierPt[1]

    let x = 0
    let y = 0
    Point2d.transformMat2d(modifierPt, screenPt, modifierMat)
    if (
      (Math.abs(modifierPt[0]) <= halfBoxSz &&
        Math.abs(modifierPt[1]) <= halfBoxSz) ||
      (Boolean(xScale) &&
        Boolean(yScale) &&
        (rotate =
          (x = xScale * modifierPt[0]) > -halfBoxSz &&
          x <= padBoxSz &&
          (y = yScale * modifierPt[1]) > -halfBoxSz &&
          y <= padBoxSz))
    ) {
      hit = true
    }
...
export default class XformShape extends Rect {
  ...
  containsPoint(screenPt) {
    ...
    if (
      (this._rotatable || this._scalable) &&
      this.visible &&
      ...
      for (let i = 0; i < Constants.SIDES; i += 1) {
        ...
        rtnObj = isPointInOBBoxModifierRect(
          screenPt,
          pt,
          this._fullXform,
          mat,
          halfBoxSz,
          this._interactiveBoxSize,
          padSz,
          padRadius,
          xScale,
          yScale,
  ...
  get width() {
    return this.parent && this.parent.width !== "undefined"
      ? this.parent.width
      : 0
  }

  get height() {
    return this.parent && this.parent.height !== "undefined"
      ? this.parent.height
      : 0
  }
  ...
  render(ctx, worldToScreenMatrix, styleState) {
    if (
      !this.parent ||
      typeof this.parent.width === "undefined" ||
