...
function inCanvas(canvas, x, y) {
  ...
  let localY = 0
  const isInCanvas =
    (localX = x - domrect.left - canvas.clientLeft) >= 0 &&
    localX <= canvas.clientWidth &&
    (localY = y - domrect.top - canvas.clientTop) >= 0 &&
    localY <= canvas.clientHeight
  return isInCanvas
}
...
function appendCustomCursor(
  _event,
  target,
  cursorStyle,
  offsetX = Constants.SVG_OFFSET,
  offsetY = Constants.SVG_OFFSET
) {
  const cursor = document.getElementById("cursor")
  ...
  if (cursor === null) {
    ...
    newCursor.setAttribute(
      "style",
      `position: absolute; top: ${`${mouse.y}px`}; left: ${`${mouse.x}px`}; width: 28px; height: 28px; background: ${cursorStyle}; cursor: none; z-index: 10; pointer-events: none; transform: translate(${offsetX}px, ${offsetY}px)`
    )
...
export default class ShapeBuilder extends DrawEngine {
  _mousedownCB(event) {
    ...
    for (i = shapes.length - 1; i >= 0; i -= 1) {
      ...
      if (shapes[i].selected) {
        ...
        if (
          ...
          if (
            selectedShape instanceof VertEditableShape &&
            event.altKey &&
            hitInfo.controlIndex < shapes[i].numVerts
          ) {
    ...
    if (i < 0 && this._selectedShapes.size) {
      ...
      selectedShape &&
      selectedInfo &&
      (selectedInfo.movable || selectedInfo.rotatable || selectedInfo.scalable)
    ) {
  ...
  _mouseupCB(event) {
    if (this._dragInfo && this._dragInfo.shape) {
      ...
      if (selectedShape && !selectedShape.selected) {
        const selectEventObj = selectShape(
          selectedShape,
          shapes,
            ...
            if (selectedShape instanceof XformShape) {
              if (hitInfo.rotate) {
                ...
                appendCustomCursor(
                  event,
                  this._parent,
                  `${rotateSvg.replace(/<degrees>/g, `${degrees}`)}`
                )
                ...
                if (hitInfo.controlIndex === 0 || hitInfo.controlIndex === 3) {
                  ...
                  appendCustomCursor(
                    ...
                    `${scaleSvg.replace(
                      /<degrees>/g,
                      `${-shapes[i].getRotation() - Constants.FORTY_FIVE_ANGLE}`
                    )}`
                  )
                } else if (
                  hitInfo.controlIndex === 1 ||
                  hitInfo.controlIndex === 2
                ) {
                  appendCustomCursor(
                    event,
                    this._parent,
                ...
                appendCustomCursor(
                  ...
                  `${scaleSvg.replace(
                    /<degrees>/g,
                    `${-shapes[i].getRotation() + Constants.RIGHT_ANGLE}`
                  )}`
                )
              }
        ...
        if (shapeInfo.editable && shapes[i] instanceof PolyLine) {
          ...
          if (doXform) {
            if (shapeInfo.rotatable || shapeInfo.scalable) {
              newSelectShape = new XformShape(
                this._xformStyle || defaultXformStyle,
                shapeInfo
              )
              selectedShape.addChildXform(newSelectShape)
            }
          } else {
            newSelectShape = new VertEditableShape(
              selectedShape,
              this._xformStyle
            )
            selectedShape.addChildXform(newSelectShape)
  ...
  _init(parent, opts) {
    ...
    const myevents = Object.getOwnPropertyNames(EventConstants).map(
      event => EventConstants[event]
    )
    this.registerEvents(myevents)
    this._dragInfo = null
    this._selectedShapes = new Map()
    this._selectStyle = new BasicStyle(
      opts && opts.selectStyle
        ? opts.selectStyle
        : {
            fillColor: "orange"
          }
    ...
    this._xformStyle = new BasicStyle(
      opts && opts.xformStyle
        ? opts.xformStyle
        : {
            fillColor: "white",
            strokeColor: "black",
  ...
  addShape(shape, opts = null, select = false) {
    ...
    shapes.forEach(newShape => {
      ...
      if (shapeInfo) {
        shapeInfo.selectable =
          opts && typeof opts.selectable !== "undefined"
            ? Boolean(opts.selectable)
            : true
        shapeInfo.movable =
          opts && typeof opts.movable !== "undefined"
            ? Boolean(opts.movable)
            : true
        shapeInfo.rotatable =
          opts && typeof opts.rotatable !== "undefined"
            ? Boolean(opts.rotatable)
            : true
        shapeInfo.scalable =
          opts && typeof opts.scalable !== "undefined"
            ? Boolean(opts.scalable)
            : true
        shapeInfo.uniformScaleOnly =
          opts && typeof opts.uniformScaleOnly !== "undefined"
            ? Boolean(opts.uniformScaleOnly)
            : false
        shapeInfo.centerScaleOnly =
          opts && typeof opts.centerScaleOnly !== "undefined"
            ? Boolean(opts.centerScaleOnly)
            : false
        shapeInfo.editable =
          opts && typeof opts.editable !== "undefined"
  ...
  deleteShape(shape) {
    ...
    const selectClearedShapes = clearSpecificShapes(
      this._selectedShapes,
      shapes
    )
    this.fire(EventConstants.SELECTION_CHANGED, {
